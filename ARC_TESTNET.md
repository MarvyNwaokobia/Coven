...
These are wired into `frontend/.env.example` as `NEXT_PUBLIC_PAYCIRCLE_CONTRACT` / `NEXT_PUBLIC_SPLIT_ESCROW_CONTRACT`. Redeploy and update both places if the contracts change.
...
- `frontend/lib/cctp/crossChainPay.ts` currently maps CCTP V1 TokenMessenger/USDC addresses per *source* chain (Ethereum, Base, Polygon, Arbitrum) for the burn leg; Arc's own TokenMessengerV2/MessageTransmitterV2 (domain 26) above are for the *mint* leg on Arc — double check V1/V2 compatibility between the two legs before mainnet.
